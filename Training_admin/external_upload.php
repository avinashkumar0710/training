...
$lastProgramId = 20252001; // Default if no data found
...
<script>
  ...
  <style>
    ...
    .custom-modal-header {
    background-color: #0b5ed7; /* Change this color as needed */
...
<h6><i class='fa fa-home'></i>&nbsp;<i><u>Training Administration->Data Insertion</u></i></h6>
<br>

<body>
<div class="container-fluid">
    <!-- <h4>Add Training Request</h4> -->
   
    <form id="training-form" action="save_request.php" method="post">
...
<button class="btn btn-primary btn-sm fixed-bottom-right" data-bs-toggle="modal" data-bs-target="#futureProgramsModal">
    ...
    <div class="modal-dialog modal-fullscreen">  <!-- Fullscreen Modal -->
        <div class="modal-content">
            <div class="modal-header custom-modal-header"> <!-- Apply custom class -->
                <h5 class="modal-title" id="futureProgramsModalLabel">Internal Edit Future Programs</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <iframe src="edit_future_programs_internal.php" style="width: 100%; height: 90vh; border: none;"></iframe>
            </div>
        </div>
    </div>
</div>
...
function fetchProgramData() {
    ...
    fetch('get_programs_internal.php')
...
async function addRow() {
    ...
    programIdInput.readOnly = true;
    programIdInput.value = lastProgramId++; // ✅ increment here
    ...
    natureOfTrainingDropdown.addEventListener('change', async function() {
        ...
        // training_mode
        newRow.insertCell().innerHTML = `<input type="text" class="form-control" name="training_mode[]" value="Internal" required style="width: 150px;">`;

        // Training Mode Dropdown (Manual Data)
    // const trainingModeCell = newRow.insertCell();
    // const trainingModeDropdown = document.createElement('select');
    // trainingModeDropdown.classList.add('form-control');
    // trainingModeDropdown.name = 'training_mode[]';
    // trainingModeDropdown.required = true;
    // trainingModeDropdown.style.width = '150px';

    // const defaultOptionMode = document.createElement('option');
    // defaultOptionMode.value = '';
    // defaultOptionMode.textContent = '-- Select --';
    // trainingModeDropdown.appendChild(defaultOptionMode);

    // const manualTrainingModes = ["Offline", "Online", "Others"];
    // manualTrainingModes.forEach(mode => {
    //     const option = document.createElement('option');
    //     option.value = mode;
    //     option.textContent = mode;
    //     trainingModeDropdown.appendChild(option);
    // });

    // trainingModeCell.appendChild(trainingModeDropdown);
