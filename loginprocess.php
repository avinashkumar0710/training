...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ✅ At this point, we know the username/password is correct.
    ...
    //     // Now check if the user is a manager
    //     $checkManager = "SELECT DISTINCT e1.rep_ofcr, e2.name 
    //                      FROM emp_mas_sap e1 
    //                      JOIN emp_mas_sap e2 ON e1.rep_ofcr = e2.empno 
    //                      WHERE e1.rep_ofcr = '$username1'";
    
    //     $stmtManager = sqlsrv_query($conn, $checkManager, array($username1)); // Use 8-digit username for Manager
    //     $managerUser = sqlsrv_fetch_array($stmtManager, SQLSRV_FETCH_ASSOC);
    
    //     // If the user is not a manager, still show "Invalid username or password"
    //     if (!$managerUser) {
    //         echo "<script>alert('Invalid username or password. Please try again.'); window.location.href='login.php';</script>";
    //         exit();
    //     }
