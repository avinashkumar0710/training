...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request'])) {
    ...
    if (isset($_POST['selectedData'])) {
        ...
        $sqlCount = "SELECT COUNT(empno) AS totalCount FROM [dbo].[request] WHERE empno = ?";
        ...
        // Check if the total exceeds 8
        if ($existingCount + $selectedCount > 8) {
            echo '<script>alert("You cannot have more than 8 requests.");</script>';
            echo '<script>window.location.href = "all_users.php";</script>';
            // Optionally redirect or exit here
    ...
    // Query to get the designation of the reporting officer
    ...
    // echo '$exactHodEmpno' .$exactHodEmpno;
    // Check if hod_ro is available in rep_ofcr or if the design is 'GM & BUH'
    // if (trim($design) == 'GM & BUH') {
    //     $flag = '4'; // Approve TNI
    // } 
    //  else
     if ($rep_ofcr == 'NULL') {
    ...
    //echo ' $flag;' . $flag;
        ...
        foreach ($selectedData as $data) {
            ...
            if ($existenceData['count'] == 0) {
                ...
                $sqlInsert = "INSERT INTO [dbo].[request] (srl_no, empno, Program_name, faculty, nature_training, year, uploaded_date, flag,  remarks, duration, tentative_date, target_group, rep_ofcr, hostel_book, plant) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $paramsInsert = array(
                    $selectedId,
                    $employeeNumber,
