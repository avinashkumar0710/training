...
<form method="POST">
    ...
    <select name="year" id="year">
        ...
        // Get available seats (no default fallback)
        $availableSeats = 0;
        ...
        if ($seatsResult && sqlsrv_has_rows($seatsResult)) {
            $seatsRow = sqlsrv_fetch_array($seatsResult, SQLSRV_FETCH_ASSOC);
            $availableSeats = (int)$seatsRow['available_seats'];
        }

        // Count existing requests
        $sqlCount = "SELECT COUNT(empno) AS total_count 
        ...
        //echo $totalCount;
        $remainingRequests = $availableSeats - $totalCount;
        
        // Only show if seats are configured
        if ($availableSeats > 0) {
            echo "<span class='badge bg-info ms-2'>Requests: $remainingRequests/$availableSeats</span>";
        }

        //sqlsrv_free_stmt($yearsResult);
        //sqlsrv_free_stmt($seatsResult);
        sqlsrv_free_stmt($stmtCount);
        sqlsrv_close($conn);
        ?>
    </select>
    <button type="submit" class="btn btn-info ms-2">Show Programs</button>
</form>&nbsp;&nbsp;&nbsp;
...
<div class="scrollable-table" style="font-size:16px;">  
...
WHERE 
    ...
    echo "<th scope='col'>Program ID</th>";
    ...
    echo "<th scope='col'>Admin Remarks</th>";
    echo "<th scope='col'>Hostel Req</th>";
    echo "<th scope='col'>Employee Remarks</th>";
...
while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    ...
    // NEW LOGIC FOR EMPLOYEE GROUP MATCHING
    $trainingEmployeeGrp = $row['Employee_grp'];


    if ($trainingEmployeeGrp == 'C') {
        // Training is for group C - show to everyone
        $gradeMatch = true;
    }
    elseif ($employee_grp == 'A' && $trainingEmployeeGrp == 'A') {
        // For group A employees - check grade match
    ...
    elseif ($employee_grp == 'B' && $trainingEmployeeGrp == 'B') {
        // For group B employees - no grade check needed
        $gradeMatch = true;
    }
    ...
    // Final disabled class override if full
    if ($seatsFull) {
        $rowClass = 'disabled-row-full';
        $disabled = 'disabled';
    }
