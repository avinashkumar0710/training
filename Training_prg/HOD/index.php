...
<body>
    ...
    echo '<h6><i class="fa fa-home"></i>&nbsp;<i><u>Training Approval</u></i></h6>';
    echo '<h3>Pending List</h3>';
    echo '<div class="table" style="height:630px; overflow-x: auto;">';
...
//     FROM [Complaint].[dbo].[request] r
//     JOIN [Complaint].[dbo].[emp_mas_sap] e ON r.empno = e.empno
//     WHERE flag = '0' AND e.hod_ro = $sessionemp";
// } else {
    // Access value is not 1, so execute the original query
    $sql = "SELECT r.Id, r.srl_no, e.name, e.empno, r.Program_name, r.Faculty, r.nature_training, r.year,  r.remarks, r.duration, r.tentative_date, r.target_group, r.rep_ofcr, e.hod_ro
            FROM [Complaint].[dbo].[request] r
            JOIN [Complaint].[dbo].[emp_mas_sap] e ON r.empno = e.empno
            WHERE flag = '0' AND e.rep_ofcr = $sessionemp";
...
//         WHERE flag in ('2') AND a.hod_ro = '$sessionemp'";
// }
// else{
    $sqlYear = "SELECT DISTINCT r.year
    FROM [Complaint].[dbo].[request] r 
...
// $sqlRecords = "SELECT r.srl_no, r.empno, r.Program_name, r.year, a.name, r.aprroved_time, r.flag, r.tentative_date
//     FROM [Complaint].[dbo].[request] r
//     JOIN [Complaint].[dbo].[emp_mas_sap] a ON r.empno = a.empno
//     WHERE flag in ('7','6','5','4','1') AND appr_empno = '$sessionemp' AND r.year = '$selectedYear'";
...
//     FROM [Complaint].[dbo].[request] r
//     JOIN [Complaint].[dbo].[emp_mas_sap] a ON r.empno = a.empno
//     WHERE flag in ('2') AND a.hod_ro = '$sessionemp' AND r.year = '$selectedYear'";
// } else {
    // Access value is not 1, so execute the original query
    $sqlRecords = "SELECT r.srl_no, r.empno, r.Program_name, r.year, a.name, r.aprroved_time, r.flag, r.tentative_date, a.hod_ro 
    FROM [Complaint].[dbo].[request] r
    JOIN [Complaint].[dbo].[emp_mas_sap] a ON r.empno = a.empno
    WHERE flag in ('1','2') AND appr_empno like ('%$sessionemp%') AND r.year = '$selectedYear'";
...
echo '</select>';
echo '&nbsp<button type="submit" class="btn btn-primary">Show</button><br><i style="font-size:small; background-color:yellow;">&nbsp;&nbsp;(Note: Please select the year first to display the Program Name.)</i>';
echo '</form>';

echo '<h3>Approve / Reject By You</h3>';
...
while ($row = sqlsrv_fetch_array($stmtRecords, SQLSRV_FETCH_ASSOC)) {
    ...
    switch ($row['flag']) {
        ...
        case 2:
            $status = '<span style="color:green">Pending at HOD';
            break;
