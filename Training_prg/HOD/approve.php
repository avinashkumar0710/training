    ...
    if (isset($_POST['approvalStatus'])) {
        ...
        foreach ($_POST['approvalStatus'] as $rowId => $selectedValue) {
            ...
            if (!empty($selectedValue)) {
                ...
                $sql = "UPDATE request SET flag = '$selectedValue', aprroved_time = GETDATE(), uploaded_date =GETDATE(), appr_empno='$hodempno' WHERE id = '$rowId'";
        ...
        echo '<script>alert("No approval statuses were submitted."); window.location.href = "index.php";</script>';
    }
} else {
    // Handle the case where the form was not submitted via POST method
    echo '<script>alert("Form was not submitted."); window.location.href = "index.php";</script>';
}
?>
