<?php 
// Start a new session
session_start();
if (!isset($_SESSION["emp_num"])) {   
    header("location:login.php");
}

$hodempno = $_SESSION["emp_num"];
 echo ''.$hodempno;

// Ensure $hodempno has 8 digits (prepend 00 if it starts with 0)
$hodempno = str_pad($hodempno, 8, "0", STR_PAD_LEFT);
?>

<?php
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['selectedIds']) && !empty($_POST['selectedIds'])) {
    $selectedIds = $_POST['selectedIds'];
    $empNo = $_SESSION['emp_num'];  // Assuming emp_num is stored in the session
    $action = isset($_POST['action']) ? $_POST['action'] : "";
    echo '<pre>';
    print_r($selectedIds);
    echo '</pre>';

    // Connect to the database
    $serverName = "192.168.100.240";
    $connectionInfo = array(
        "Database" => "complaint",
        "UID" => "sa",
        "PWD" => "Intranet@123"   // Add your password if necessary
    );

    $conn = sqlsrv_connect($serverName, $connectionInfo);
    if ($conn === false) {
        die(print_r(sqlsrv_errors(), true));
    }

    // Loop through each selected ID
    if (!empty($selectedIds)) {
        foreach ($selectedIds as $id) {
          // Validate and sanitize the input as needed
          $id = filter_var($id, FILTER_SANITIZE_NUMBER_INT);
    
          $sql = ""; // Initialize empty query string
          $params = []; // Initialize empty parameter array
    
          if ($action === "approve") {
            // Update query for approval (flag = 2)
            $sql = "UPDATE [Complaint].[dbo].[request_TNI] set flag = '2', aprroved_time = GETDATE(), appr_empno = '$hodempno' WHERE id = '$id'";
            $params = array($id);
            //echo 'approve';
          } elseif ($action === "reject") {
            // Update query for rejection (flag = 1)
            $sql = "UPDATE [Complaint].[dbo].[request_TNI] set flag = '1', aprroved_time = GETDATE(), appr_empno = '$hodempno' WHERE id = '$id'";
            $params = array($id);
            //echo 'reject';
          } else {
            //echo 'invalid';
            // Handle invalid action (optional)
            //echo '<script>alert("Invalid action!"); window.location.href = "TNI_approval.php";</script>';
            exit();
          }
    
    
          $stmt = sqlsrv_query($conn, $sql, $params);
          if ($stmt === false) {
            die(print_r(sqlsrv_errors(), true));
          }
        }
      } 

    // Close the database connection
    sqlsrv_close($conn);

    echo '<script>alert("Form submitted successfully!"); window.location.href = "TNI_approval.php";</script>';
} else {
    // Handle the case where no checkboxes were selected
    echo '<script>alert("Please select at least one TNI request to approve."); window.location.href = "TNI_approval.php";</script>';
}
?>
