...
try {
    ...
    $sql = "SELECT
            ...
            WHERE $flagCondition AND act_Nact_flag = '1'
            ORDER BY ar.from_date desc";
    ...
    header('Content-Disposition: attachment;filename="training_details_data.xls"');
    ...
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        echo '</tr>';
        $totalManDays += (int) $row['ManDays']; // Accumulate total mandays
    }

    // Output total mandays row at the end
    echo '<tr>';
    echo '<td colspan="18   " style="text-align: right; font-weight: bold;">Grand Total ManDays:</td>';
